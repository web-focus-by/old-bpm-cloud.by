...
const ThirdMobailMenu = ({ items }) => {
  ...
  const renderItems = items.routes.map((elem, i) => {
    ...
    const renerContent = childLength ? (
      <div key={elem.id} onMouseDown={() => setActive(i)}>
        {elem.title}
      </div>
    ) : (
      <div key={elem.id}>
  ...
  return (
    <div className={style.wrapper}>
      <div
        ...
        onMouseDown={() => ShowFourtItems(!isShowFourtItems)}
      >
        {renderItems}
      </div>
      <div
        className={classnames(style.mobileMenuWrapper, {
          [style.showMobileMenu]: isShowFourtItems,
        })}
      >
        <div
          className={style.goBackWrapper}
          onMouseDown={() => ShowFourtItems(!isShowFourtItems)}
        >
          <div>
            <svg
        ...
        <div className={style.cardTitle}>
          <Link to={items.routes[active].path}>
            {items.routes[active].title}
          </Link>
        </div>
